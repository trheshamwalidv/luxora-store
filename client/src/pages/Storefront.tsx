...
function HomeView({ snapshot, products }: { snapshot: Snapshot; products: Product[] }) {
  const [, setLocation] = useLocation();
  const featured = products.filter(product => product.isFeatured).sort((a, b) => { const rank = (product: Product) => product.slug === "blue-shift-shell" ? 0 : product.category === "sneakers" ? 1 : product.category === "gym" ? 2 : 3; return rank(a) - rank(b); }).slice(0, 8);
...
function ShopView({ snapshot, products, defaultCategory, collection }: { snapshot: Snapshot; products: Product[]; defaultCategory?: Product["category"]; collection?: Collection }) {
  const [, setLocation] = useLocation();
  const collectionCategory = collection?.category === "featured" ? undefined : collection?.category;
  const initialCategory = defaultCategory ?? collectionCategory;
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<Product["category"] | "all">(initialCategory ?? "all");
  ...
  const reset = () => { setQuery(""); setCategory(initialCategory ?? "all"); setSize("all"); setColor("all"); setMaxPrice(350000); };
  return <main className="min-h-screen bg-[#111] pb-20"><div className="container py-10 sm:py-16"><div className="border-b border-white/10 pb-7 sm:pb-10">{collection?.imageUrl ? <div className="mb-7 aspect-[16/7] overflow-hidden bg-[#272727]"><img src={collection.imageUrl} alt={collection.title} className="h-full w-full object-cover" /></div> : null}<p className="text-[10px] font-extrabold tracking-[0.15em] text-[var(--lux-acid)]">{collection ? `COLLECTION / ${collection.slug.toUpperCase()}` : "THE STORE / EDIT 01"}</p><div className="mt-3 flex flex-wrap items-end justify-between gap-4"><div><h1 className="font-display text-5xl font-black tracking-[-0.065em] text-white sm:text-7xl">{collection?.title ?? "المتجر."}</h1>{collection?.description ? <p className="mt-4 max-w-xl text-sm leading-7 text-white/60">{collection.description}</p> : null}</div><span className="text-xs font-bold text-white/45">{filtered.length} قطع متاحة</span></div></div>
...
function CheckoutPage({ settings, cart, onBack, clearCart }: { settings: StoreSettings; cart: CartLine[]; onBack: () => void; clearCart: () => void }) {
  ...
  const createOrder = trpc.orders.create.useMutation({
    onSuccess: result => { setCompletedOrder(result.orderNumber); clearCart(); toast.success("تم تسجيل طلبك بنجاح."); },
    onError: error => toast.error(error.message),
  });
  const submitOrder = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!cart.length) return;
    createOrder.mutate({ customerName: customer.name, phone: customer.phone, address: customer.address, notes: customer.notes || null, items: cart.map(line => ({ productId: line.product.id, variantId: line.variant.id, quantity: line.quantity })) });
  };
  if (completedOrder) return <main className="grid min-h-screen place-items-center bg-[#f4f4f2] px-5 text-center text-[#171717]"><div className="w-full max-w-lg rounded-sm bg-white p-8 shadow-sm sm:p-12"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[#dfff00]"><Check className="h-7 w-7" /></div><p className="mt-7 text-[10px] font-black tracking-[0.16em] text-black/45">ORDER CONFIRMED</p><h1 className="mt-2 font-display text-4xl font-black tracking-[-0.05em]">طلبك اتسجل.</h1><p className="mt-4 text-sm leading-7 text-black/60">رقم الطلب <strong className="text-black">{completedOrder}</strong>. هنراجع بيانات التوصيل ونتواصل معاك لتأكيد الدفع عند الاستلام.</p><button onClick={onBack} className="mt-8 h-12 w-full bg-black text-sm font-black text-white transition hover:bg-[#dfff00] hover:text-black">العودة للمتجر</button></div></main>;
  if (!cart.length) return <main className="grid min-h-screen place-items-center bg-[#f4f4f2] px-5 text-center text-[#171717]"><div><p className="font-display text-3xl font-black">السلة فاضية.</p><button onClick={onBack} className="mt-6 h-12 bg-black px-7 text-sm font-black text-white">العودة للمتجر</button></div></main>;
  return <main dir="rtl" className="min-h-screen bg-[#f4f4f2] text-[#171717]"><header className="border-b border-black/10 bg-white"><div className="mx-auto flex h-20 max-w-6xl items-center justify-between px-5 sm:px-8"><button onClick={onBack} className="text-xs font-bold text-black/55 transition hover:text-black">العودة للسلة</button><div className="text-center"><p className="font-display text-2xl font-black tracking-[0.12em]">{settings.brandName}</p><p className="mt-1 text-[9px] font-bold tracking-[0.18em] text-black/40">SECURE CHECKOUT</p></div><span className="text-[10px] font-bold text-black/45">الدفع عند الاستلام</span></div></header><div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 sm:py-10"><div className="mb-8 flex items-center gap-2 text-[11px] font-bold text-black/45"><span className="text-black">السلة</span><ChevronLeft className="h-3 w-3" /><span className="text-black">بيانات التوصيل</span><ChevronLeft className="h-3 w-3" /><span>تأكيد الطلب</span></div><div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_390px] lg:gap-14"><form onSubmit={submitOrder} className="order-2 space-y-7 lg:order-1"><section className="rounded-sm bg-white p-5 shadow-sm sm:p-7"><div className="flex items-baseline justify-between gap-3"><h1 className="font-display text-3xl font-black tracking-[-0.05em]">بيانات التوصيل</h1><span className="text-[10px] font-bold text-black/45">الخطوة 1 من 1</span></div><p className="mt-2 text-xs leading-6 text-black/55">اكتب بياناتك، وهنراجع الطلب معاك قبل التسليم.</p><div className="mt-7 space-y-4"><label className="block"><span className="mb-2 block text-xs font-bold">الاسم بالكامل</span><input required value={customer.name} onChange={event => setCustomer({ ...customer, name: event.target.value })} placeholder="الاسم بالكامل" className="h-13 w-full rounded-sm border border-black/25 bg-white px-4 text-sm outline-none transition focus:border-black" /></label><label className="block"><span className="mb-2 block text-xs font-bold">رقم الموبايل</span><input required type="tel" value={customer.phone} onChange={event => setCustomer({ ...customer, phone: event.target.value })} placeholder="01xxxxxxxxx" className="h-13 w-full rounded-sm border border-black/25 bg-white px-4 text-sm outline-none transition focus:border-black" /></label><label className="block"><span className="mb-2 block text-xs font-bold">العنوان بالتفصيل</span><textarea required value={customer.address} onChange={event => setCustomer({ ...customer, address: event.target.value })} placeholder="المحافظة، المنطقة، الشارع، رقم المبنى" className="min-h-28 w-full resize-none rounded-sm border border-black/25 bg-white p-4 text-sm outline-none transition focus:border-black" /></label><label className="block"><span className="mb-2 block text-xs font-bold">ملاحظات إضافية <em className="font-normal text-black/40">(اختياري)</em></span><textarea value={customer.notes} onChange={event => setCustomer({ ...customer, notes: event.target.value })} placeholder="أي تفاصيل تساعدنا في التوصيل" className="min-h-20 w-full resize-none rounded-sm border border-black/25 bg-white p-4 text-sm outline-none transition focus:border-black" /></label></div></section><section className="rounded-sm bg-white p-5 shadow-sm sm:p-7"><h2 className="text-lg font-black">طريقة الدفع</h2><div className="mt-4 flex items-center justify-between rounded-sm border-2 border-black bg-[#fafafa] p-4"><div><p className="text-sm font-black">الدفع عند الاستلام</p><p className="mt-1 text-xs text-black/50">مفيش دفع إلكتروني دلوقتي</p></div><span className="grid h-5 w-5 place-items-center rounded-full border-4 border-black bg-white" /></div><button type="submit" disabled={createOrder.isPending} className="mt-6 flex h-14 w-full items-center justify-center gap-3 rounded-sm bg-black text-sm font-black text-white transition hover:bg-[#dfff00] hover:text-black disabled:opacity-50">{createOrder.isPending ? "جارٍ تسجيل الطلب..." : "تأكيد الطلب"}<ArrowLeft className="h-4 w-4" /></button><p className="mt-3 text-center text-[10px] leading-5 text-black/45">بالضغط على تأكيد الطلب، أنت بتطلب الدفع عند الاستلام فقط.</p></section></form><aside className="order-1 h-fit rounded-sm bg-[#e9e9e6] p-5 lg:order-2 lg:p-7"><h2 className="text-lg font-black">ملخص الطلب</h2><div className="mt-5 space-y-4">{cart.map(line => <div key={line.variant.id} className="flex gap-3"><div className="relative h-20 w-16 shrink-0 overflow-hidden rounded-sm bg-[#d5d5d1]"><img src={getMainImage(line.product)} alt={line.product.name} className="h-full w-full object-cover" /><span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-black px-1 text-[10px] font-bold text-white">{line.quantity}</span></div><div className="min-w-0 flex-1"><p className="text-sm font-black">{line.product.name}</p><p className="mt-1 text-[10px] text-black/50">{line.variant.colorName} / {line.variant.size}</p></div><p className="text-sm font-black">{price(line.product.priceCents * line.quantity)}</p></div>)}</div><div className="my-6 border-t border-black/15" /><div className="space-y-3 text-xs"><div className="flex justify-between"><span className="text-black/55">الإجمالي الفرعي</span><span className="font-bold">{price(total)}</span></div><div className="flex justify-between"><span className="text-black/55">الشحن</span><span className="font-bold">يتم تأكيده مع الطلب</span></div></div><div className="my-6 border-t border-black/15" /><div className="flex items-end justify-between"><span className="text-sm font-black">الإجمالي</span><strong className="font-display text-2xl font-black">{price(total)}</strong></div><p className="mt-5 border border-black/10 bg-white/50 p-3 text-[10px] leading-5 text-black/55">الدفع عند الاستلام — هنراجع بياناتك ونتواصل معاك قبل تجهيز الطلب.</p></aside></div></div></main>;
}

function CartPanel({ cart, open, onClose, updateQuantity, remove, goToCheckout }: { cart: CartLine[]; open: boolean; onClose: () => void; updateQuantity: (id: string, quantity: number) => void; remove: (id: string) => void; goToCheckout: () => void }) {
  if (!open) return null;
  return <div className="fixed inset-0 z-[80] bg-black/65 backdrop-blur-sm" onClick={onClose}><aside className="mr-auto flex h-full w-full max-w-md flex-col bg-[#151515] text-white shadow-2xl" onClick={event => event.stopPropagation()}><div className="flex items-center justify-between border-b border-white/10 p-5"><div><p className="font-display text-2xl font-black">سلة الشراء</p><p className="mt-1 text-[10px] font-bold tracking-wider text-white/45">{cart.length} منتجات مختارة</p></div><button onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full border border-white/15"><X className="h-4 w-4" /></button></div><div className="flex-1 overflow-y-auto p-5">{cart.length ? <div className="space-y-4">{cart.map(line => { const atLimit = line.quantity >= line.variant.stockQuantity; return <div className="flex gap-3 border-b border-white/10 pb-4" key={line.variant.id}><img src={getMainImage(line.product)} alt={line.product.name} className="h-24 w-20 object-cover" /><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><div><p className="text-sm font-bold text-white">{line.product.name}</p><p className="mt-1 text-[10px] text-white/45">{line.variant.colorName} / {line.variant.size}</p></div><button onClick={() => remove(line.variant.id)} aria-label={"حذف " + line.product.name} className="h-7 text-white/45 hover:text-[var(--lux-acid)]"><Trash2 className="h-4 w-4" /></button></div><div className="mt-4 flex items-center justify-between"><p className="text-xs font-black">{price(line.product.priceCents * line.quantity)}</p><div className="flex items-center border border-white/20"><button onClick={() => updateQuantity(line.variant.id, line.quantity - 1)} className="grid h-7 w-7 place-items-center"><Minus className="h-3 w-3" /></button><span className="grid h-7 w-7 place-items-center text-xs font-bold">{line.quantity}</span><button onClick={() => updateQuantity(line.variant.id, line.quantity + 1)} disabled={atLimit} className="grid h-7 w-7 place-items-center disabled:text-white/25"><Plus className="h-3 w-3" /></button></div></div>{atLimit ? <p className="mt-2 text-[10px] font-bold text-amber-300">وصلت للحد الأقصى المتاح لهذا المقاس: {line.variant.stockQuantity} قطع.</p> : null}</div></div>; })}</div> : <div className="grid h-full min-h-[300px] place-items-center text-center"><div><ShoppingBag className="mx-auto h-8 w-8 text-white/25" /><p className="mt-4 font-display text-2xl font-black">السلة فاضية.</p></div></div>}</div>{cart.length ? <div className="border-t border-white/10 p-5"><div className="mb-4 flex items-center justify-between"><span className="text-sm font-bold text-white/60">الإجمالي</span><strong className="text-xl font-black">{price(cart.reduce((sum, line) => sum + line.product.priceCents * line.quantity, 0))}</strong></div><button onClick={goToCheckout} className="flex h-14 w-full items-center justify-center gap-3 bg-[var(--lux-acid)] text-sm font-black text-black">إتمام الطلب بأمان <ArrowLeft className="h-4 w-4" /></button><p className="mt-3 text-center text-[10px] leading-5 text-white/40">الدفع عند الاستلام — بدون دفع إلكتروني.</p></div> : null}</aside></div>;
}
...
export default function Storefront() {
  ...
  if (path === "/checkout") return <div dir="rtl" className="min-h-screen"><CheckoutPage settings={snapshot.settings} cart={cart} onBack={() => setLocation("/")} clearCart={() => setCart([])} /></div>;
  ...
  else if (path.startsWith("/collections/")) { const slug = path.split("/").pop() ?? ""; const collection = snapshot.collections.find(item => item.slug === slug); page = <ShopView snapshot={snapshot} products={products} defaultCategory={collection?.category === "featured" ? undefined : collection?.category} collection={collection} />; }
