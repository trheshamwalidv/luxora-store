...
function CartPanel({ settings, cart, open, onClose, updateQuantity, remove, clearCart }: { settings: StoreSettings; cart: CartLine[]; open: boolean; onClose: () => void; updateQuantity: (id: string, quantity: number) => void; remove: (id: string) => void; clearCart: () => void }) {
  ...
  const submitOrder = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (hasStockLimit) return toast.error("قلل الكمية قبل تأكيد الطلب."); createOrder.mutate({ customerName: customer.name, phone: customer.phone, address: customer.address, notes: customer.notes || null, items: cart.map(line => ({ productId: line.product.id, variantId: line.variant.id, quantity: line.quantity })) }); };
  if (!open) return null;
  return <div className="fixed inset-0 z-[80] bg-black/65 backdrop-blur-sm" onClick={onClose}>
    <aside className="mr-auto flex h-full w-full max-w-md flex-col bg-[#151515] text-white shadow-2xl" onClick={event => event.stopPropagation()}>
      ...
      {cart.length ? (orderOpen ? <form onSubmit={submitOrder} className="border-t border-white/10 p-5"><div className="mb-5 flex items-center gap-3"><button type="button" onClick={() => setOrderOpen(false)} className="grid h-9 w-9 place-items-center rounded-full border border-white/15"><ChevronRight className="h-4 w-4" /></button><div><p className="font-display text-2xl font-black">بيانات الطلب</p><p className="mt-1 text-[10px] font-bold text-white/45">الإجمالي {price(total)}</p></div></div><p className="mb-5 text-xs leading-6 text-white/60">اكتب بيانات التوصيل، وهنراجع الطلب معاك قبل الدفع.</p><div className="space-y-3"><input required value={customer.name} onChange={event => setCustomer({ ...customer, name: event.target.value })} placeholder="الاسم بالكامل" className="h-12 w-full border border-white/15 bg-white/5 px-3 text-sm text-white outline-none placeholder:text-white/35 focus:border-[var(--lux-acid)]" /><input required type="tel" value={customer.phone} onChange={event => setCustomer({ ...customer, phone: event.target.value })} placeholder="رقم الموبايل" className="h-12 w-full border border-white/15 bg-white/5 px-3 text-sm text-white outline-none placeholder:text-white/35 focus:border-[var(--lux-acid)]" /><textarea required value={customer.address} onChange={event => setCustomer({ ...customer, address: event.target.value })} placeholder="العنوان بالتفصيل" className="min-h-24 w-full resize-none border border-white/15 bg-white/5 p-3 text-sm text-white outline-none placeholder:text-white/35 focus:border-[var(--lux-acid)]" /><textarea value={customer.notes} onChange={event => setCustomer({ ...customer, notes: event.target.value })} placeholder="ملاحظات إضافية (اختياري)" className="min-h-20 w-full resize-none border border-white/15 bg-white/5 p-3 text-sm text-white outline-none placeholder:text-white/35 focus:border-[var(--lux-acid)]" /></div><button type="submit" disabled={createOrder.isPending} className="mt-5 flex h-14 w-full items-center justify-center gap-3 bg-[var(--lux-acid)] text-sm font-black text-black disabled:opacity-60">{createOrder.isPending ? "جارٍ تسجيل الطلب..." : "تأكيد الطلب"} <ArrowLeft className="h-4 w-4" /></button><p className="mt-3 text-center text-[10px] leading-5 text-white/40">الدفع عند الاستلام. هنراجع بيانات التوصيل ونتواصل معاك لتأكيد الطلب.</p></form> : <div className="border-t border-white/10 p-5">{hasStockLimit ? <p className="mb-3 border border-amber-300/35 bg-amber-300/10 px-3 py-2 text-center text-[10px] font-bold leading-5 text-amber-200">بعض المقاسات وصلت للكمية المتاحة؛ قلل الكمية لتعديل السلة.</p> : null}<div className="mb-4 flex items-center justify-between"><span className="text-sm font-bold text-white/60">الإجمالي</span><strong className="text-xl font-black">{price(total)}</strong></div><button onClick={() => setOrderOpen(true)} disabled={hasStockLimit} className="flex h-14 w-full items-center justify-center gap-3 bg-[var(--lux-acid)] text-sm font-black text-black disabled:opacity-60">إتمام الطلب بأمان <ArrowLeft className="h-4 w-4" /></button><p className="mt-3 text-center text-[10px] leading-5 text-white/40">الدفع عند الاستلام — بدون دفع إلكتروني.</p></div>) : null}
    </aside>
  </div>;
}
